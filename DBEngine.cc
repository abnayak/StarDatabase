...
char *binfilespath = "/home/abhijeet/NetBeansProjects/dbfile_dir/";
char *tblfilespath = "/cise/tmp/dbi_sp11/DATA/1G/";
...
bool MyParser::ParseQuery() {
    ...
    //reverse the order of tables received from the query
    while (ptr != NULL) {
        temp = ptr->next;
        ptr->next = previous;
        previous = ptr;
...
void MyQueryRunner::InorderPrint(QueryPlanNode *root) {
    ...
    cout << "***********" << endl;
    cout << enumvals[root->opType] << " Operation";
    cout << "\n Input Pipe ID " << root->inputPipe1;
    cout << "\n Input pipe ID " << root->inputPipe2;
    cout << "\n Output pipe ID " << root->outPipe;
