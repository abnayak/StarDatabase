...
; //stores the attributes and its types if Create Table Command is called or NULL
extern bool DDL;

enum QueryOps {
    ProjectOp, GroupByOp, SumOp, JoinOp, DistinctOp, SelectFileOp, SelectPipeOp, CreateOp, DropOp, InsertOp, SetOp
};
...
// Represent each attribute in catalog
class CatAttribs {
public:
    string attrib;
    string type;
...
// Used to initiate the catalog file
...
public:
    ...
    //Storage the catalog file in the hash table
