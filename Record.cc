...
void Record::MergeRecords(Record *left, Record *right, int numAttsLeft, int numAttsRight, int *attsToKeep, int numAttsToKeep, int startOfRight) {
    ...
    for (int i = 0; i < numAttsToKeep; i++) {
        ...
        //cout << "for copy done" << endl;
    }
    
    //cout << "1. copy done" << endl;
...
void Record::Print(Schema *mySchema) {
    ...
    /*cout << "==============================" << endl;
    for ( int j =0; j <= n ; j++){
            cout << "j: " << j << "Value : " << *(int*)&bits[sizeof(int)*j] << endl;
    }
    cout << "==============================" << endl;*/
...
int Record::WriteToFile(FILE *file, Schema *mySchema) {
    ...
    //return non -ve no on success and EOF on failure
    return (fputs((ss.str()).c_str(), file));
}

off_t Record::GetSize() {
    return ((int*) bits)[0];
}
