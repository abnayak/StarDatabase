...
void OrderMaker::SaveToFile(std::ofstream &out) {
    out << numAtts << "\n";
    for (int i = 0; i < numAtts; i++) {
        out << whichAtts[i] << " " << whichTypes[i] << "\n";
    }
}

void OrderMaker::Initialize(int NumAtts, int *WhichAtts, Type *WhichTypes) {
    //If NumAtts is greater than 0 then initialize
    //If it is 0 then reset the order makder
    if (NumAtts > 0) {
        numAtts = NumAtts;
        for (int i = 0; i < numAtts; i++) {
            whichAtts[i] = WhichAtts[i];
            whichTypes[i] = WhichTypes[i];
        }
    } else {
        numAtts = 0;
